...
/*
* Java不允许在运行时改变程序结构或类型变量的结构，但它允许在运行时去探知、加载、调用在编译期完全未知的class，
* 可以在运行时加载该class，生成实例对象（instance object），调用method，或对field赋值。
* 这种类似于“看透”了class的特性被称为反射（Reflection），
* 我们可以将反射直接理解为：可以看到自己在水中的倒影，这种操作与直接操作源代码效果相同，但灵活性高得多
* */
...
public class ToyReflection {
    ...
    public static void main(String[] args) {
        ...
        try {
            // 获得类对象
            Class<?> c = Class.forName("ThinkingInJava.Reflect.Toy");
            printInfo("获得类对象", c);
